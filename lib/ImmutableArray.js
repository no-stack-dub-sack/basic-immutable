const { cloneDeep, isEqual, pull } = require('lodash');
const { baseAsMutable, baseToObject, baseGet, baseSet, baseUpdate, errors } = require('./utils/index');
...
function ImmutableArray(immutable, freeze) {
    ...
    // shorthand for Array.splice(index, 1), returns new
    Object.defineProperty(immutable.array, 'delete', {
        value: function (index) {
            if (Math.abs(index) > immutable.array.length - 1) {
                throw new Error(errors.INDEX_SCOPE);
            }
            const copy = cloneDeep(immutable.array);
            copy.splice(index, 1);
            return Immutable(copy, true);
    ...
    Object.defineProperty(immutable.array, 'toObject', {
        value: function (keyOrKeyGenFunc) {
            return baseToObject(immutable.array, keyOrKeyGenFunc);
        }
    });
